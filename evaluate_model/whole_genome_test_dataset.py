...
for x in os.listdir(sys.argv[1]):
    if not (x.startswith(sys.argv[2]) and x.endswith("gz")):
        continue
    file_list.append(x)
    #print x

genome_write = gzip.open(sys.argv[1] + "whole_genome_sequences.gz", "wt")

for file_name in file_list:
    print(file_name)

    file_read = gzip.open(sys.argv[1] + file_name, "rt")
