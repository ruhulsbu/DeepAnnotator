...
x_data_pos=np.array([[randint(1,2) for p in range(101)] for i in range(max_data_size)])
print(x_data_pos.shape, np.average(x_data_pos), np.min(x_data_pos))

x_data_neg=np.array([[randint(3,4) for p in range(101)] for i in range(max_data_size)])
print(x_data_neg.shape, np.average(x_data_neg), np.max(x_data_neg))
"""
"""
#Process Data Index

train_index = int((len(x_data_neg) / batch_size) * 0.60 * batch_size)
eval_index = train_index + int((len(x_data_neg) / batch_size) * 0.20 * batch_size)
test_index = eval_index + int((len(x_data_neg) / batch_size) * 0.20 * batch_size)
